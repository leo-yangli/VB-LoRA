...
class ModelArguments:
    ...
    topk: Optional[int] = field(
        default=3,
        metadata={"help": "TopK vector routing"},
    )
    num_vectors: Optional[int] = field(
        default=20,
        metadata={"help": "Number of vectors in vector bank"},
    )
    vector_length: Optional[int] = field(
        default=768,
        metadata={"help": "The length of each vector in vector bank"},
    )
    rank: Optional[int] = field(
        default=None,
        metadata={"help": "The max rank. It's also the number of routers"},
    )
    vb_module: Optional[str] = field(
        default="query,key,value,intermediate,layer.output,attention.output",
        metadata={"help": "The modules: query,key,value,intermediate,layer.output,attention.output"},
    )
...
def main():
    ...
    training_args.logging_dir = training_args.output_dir + '/log'
    ...
    config = AutoConfig.from_pretrained(
        ...
        vb_module = model_args.vb_module,
        topk = model_args.topk,
        num_vectors=model_args.num_vectors,
        vector_length=model_args.vector_length,
    ...
    trainable_params =['vector_bank', 'logits']
            ...
            if name.startswith('deberta') or name.startswith('roberta'):
                ...
                for trainable_param in trainable_params:
                    if trainable_param in name:
                        ...
                        logger.info(f'{name}, {sub_num_param}')
                        break
            else:
                param.requires_grad = True
    else:
