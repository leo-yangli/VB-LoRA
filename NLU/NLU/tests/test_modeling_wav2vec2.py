...
class Wav2Vec2ModelTester:
    ...
    def check_ctc_loss(self, config, input_values, *args):
        ...
        self.parent.assertTrue(
            abs(labels.shape[0] * labels.shape[1] * mean_loss.item() - sum_loss.item())
            < 1e-3
        )
...
class Wav2Vec2RobustModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (Wav2Vec2ForCTC, Wav2Vec2Model, Wav2Vec2ForMaskedLM)
        if is_torch_available()
        else ()
    )
    test_pruning = False
    ...
    def test_compute_mask_indices_overlap(self):
        ...
        for batch_sum in mask.sum(axis=-1):
            self.assertIn(
                ...
                list(
                    range(
                        int(mask_prob // mask_length * sequence_length),
        ...
        for batch_sum in mask.sum(axis=-1):
            self.assertIn(
                ...
                list(
                    range(
                        int(mask_prob // mask_length * sequence_length // 2),
                        int(mask_prob * sequence_length // 2),
                    )
                ),
            )
...
class Wav2Vec2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_ctc_normal_batched(self):
        ...
        inputs = processor(
            input_speech, return_tensors="pt", padding=True, truncation=True
        )
