...
class CustomInputPipelineCommonMixin:
    ...
    valid_inputs = (
        VALID_INPUTS  # Some inputs which are valid to compare fast and slow tokenizers
    )
    ...
    def _compare_slow_fast_pipelines(
        self, nlp_slow: Pipeline, nlp_fast: Pipeline, method: str
    ):
        ...
        with mock.patch.object(
            ...
            nlp_fast.model, method, wraps=getattr(nlp_fast.model, method)
        ) as mock_fast:
            for inputs in self.valid_inputs:
                ...
                self.assertEqual(
                    len(mock_slow.call_args_list), len(mock_fast.call_args_list)
                )
                for mock_slow_call_args, mock_fast_call_args in zip(
                    ...
                    slow_call_args, slow_call_kwargs = (
                        to_py_obj(slow_call_args),
                        to_py_obj(slow_call_kwargs),
                    )
                    fast_call_args, fast_call_kwargs = (
                        to_py_obj(fast_call_args),
                        to_py_obj(fast_call_kwargs),
                    )
...
class MonoInputPipelineCommonMixin(CustomInputPipelineCommonMixin):
    ...
    def _test_pipeline(self, nlp: Pipeline):
        ...
        multi_result = [
            nlp(input, **self.pipeline_running_kwargs) for input in self.valid_inputs
        ]
