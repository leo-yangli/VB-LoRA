    ...
    if "scheduler" in config:
        ...
        if optimizer is not None:
            lr_scheduler = trainer.create_scheduler(
                optimizer=optimizer, num_training_steps=num_training_steps
            )
...
class TensorBoardCallback(TrainerCallback):
    ...
    def __init__(self, tb_writer=None):
        ...
        assert has_tensorboard, "TensorBoardCallback requires tensorboard to be installed. Either update your PyTorch version or install tensorboardX."
        if has_tensorboard:
            try:
...
class AzureMLCallback(TrainerCallback):
    ...
    def __init__(self, azureml_run=None):
        assert is_azureml_available(), "AzureMLCallback requires azureml to be installed. Run `pip install azureml-sdk`."
