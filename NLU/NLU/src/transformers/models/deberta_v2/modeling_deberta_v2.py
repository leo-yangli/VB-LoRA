...
class DisentangledSelfAttention(torch.nn.Module):
    ...
    def manually_gather(self, input, index):
        ...
        index = index + torch.arange(
            start=0,
            end=index.size(0) * input.shape[-1],
            step=input.shape[-1],
            device=index.device,
        ).view(-1, 1)
        return torch.index_select(
            input.view(input.shape[0], -1), dim=-1, index=index.view(-1)
        ).view(input.shape[0], index.shape[0], index.shape[1])
    ...
    def disentangled_attention_bias(
        ...
        if self.share_att_key:
            pos_query_layer = self.transpose_for_scores(
                nn.Linear.forward(self.query_proj, rel_embeddings),
                self.num_attention_heads,
        ...
        if "p2c" in self.pos_att_type:
            ...
            p2c_att = self.manually_gather(
                p2c_att, p2c_pos.squeeze(0).squeeze(0)
            ).transpose(-1, -2)
            if query_layer.size(-2) != key_layer.size(-2):
    ...
    def forward(
        ...
        if self.masking_prob > 0:
            random_indices = torch.bernoulli(
                torch.full(input_ids.shape, self.masking_prob, device=input_ids.device)
            ).bool()
            masking_indces = (
        ...
        if labels is not None:
            if labels.dim() == 1 or labels.size(-1) == 1:
                ...
                if label_index.size(0) > 0:
                    ...
                    p12_loss = F.kl_div(
                        F.log_softmax(p1_logits, dim=-1),
                        F.softmax(p2_logits, dim=-1),
                        reduction="mean",
                    )
                    p21_loss = F.kl_div(
                        F.log_softmax(p2_logits, dim=-1),
                        F.softmax(p1_logits, dim=-1),
                        reduction="mean",
                    )
                else:
                    p1_loss = torch.tensor(0).to(p1_logits)
    ...
    def forward(
        ...
        if self.reg_loss_wgt > 0:
            ...
            if self.masking_prob > 0:
                random_indices = torch.bernoulli(
                    torch.full(
                        input_ids.shape, self.masking_prob, device=input_ids.device
                    )
                ).bool()
                masking_indces = (
                    (input_ids != self.cls_token_id)
