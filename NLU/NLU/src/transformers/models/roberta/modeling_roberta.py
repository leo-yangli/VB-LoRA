...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if  "query" in config.vb_module:
            self.query = VBLinear(in_features=config.hidden_size, out_features=self.all_head_size, \
                                         vector_length=config.vector_length, num_vectors=config.num_vectors, rank=config.rank, topk=config.topk)
        else:
            self.query = nn.Linear(config.hidden_size, self.all_head_size)
        
        if "key" in config.vb_module:
            self.key = VBLinear(in_features=config.hidden_size, out_features=self.all_head_size, \
                                         vector_length=config.vector_length, num_vectors=config.num_vectors, rank=config.rank, topk=config.topk)
        else:
            self.key = nn.Linear(config.hidden_size, self.all_head_size)

        if "value" in config.vb_module:
            self.value = VBLinear(in_features=config.hidden_size, out_features=self.all_head_size, \
                                         vector_length=config.vector_length, num_vectors=config.num_vectors, rank=config.rank, topk=config.topk)
        else:
...
class RobertaSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        if "attention.output" in config.vb_module:
            self.dense = VBLinear(in_features=config.hidden_size, out_features=config.hidden_size, \
                                         vector_length=config.vector_length, num_vectors=config.num_vectors, rank=config.rank, topk=config.topk)
        else:
...
class RobertaIntermediate(nn.Module):
    def __init__(self, config, ):
        super().__init__()
        if "intermediate" in config.vb_module:
            self.dense = VBLinear(in_features=config.hidden_size, out_features=config.intermediate_size, \
                                         vector_length=config.vector_length, num_vectors=config.num_vectors, rank=config.rank, topk=config.topk)
        else:
...
class RobertaOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        if "layer.output" in config.vb_module:
            self.dense = VBLinear(in_features=config.intermediate_size, out_features=config.hidden_size, \
                                         vector_length=config.vector_length, num_vectors=config.num_vectors, rank=config.rank, topk=config.topk)
        else:
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.init_weights()
        # tied vector bank, init logits
        self.vector_bank = nn.Parameter(torch.empty(config.num_vectors, config.vector_length))
        torch.nn.init.uniform_(self.vector_bank, -0.02, 0.02)
        # torch.nn.init.normal_(self.vector_bank, 0, 0.05)
        for module in self.modules():
            if isinstance(module, VBLinear):
                module.vector_bank = self.vector_bank
                torch.nn.init.normal_(module.logits_A, 0, 0.01)
                torch.nn.init.normal_(module.logits_B, 0, 0.01)
                if hasattr(module, 'logits_noise_A'):
                    torch.nn.init.normal_(module.logits_noise_A, 0, 0.01)
                    torch.nn.init.normal_(module.logits_noise_B, 0, 0.01)
                # torch.nn.init.normal_(module.logits_noise_A, 0, 0.01)
                # torch.nn.init.normal_(module.logits_noise_B, 0, 0.01)
