...
def get_peft_model_state_dict(
    ...
    if config.peft_type in (PeftType.LORA, PeftType.ADALORA):
        ...
        if config.save_topk_logits:
            to_return = {k+'_topk_indices': state_dict[k].topk(config.topk)[0] for k in state_dict if "vblora_logits" in k}
            to_return.update({k+'_topk_values': state_dict[k].topk(config.topk)[1] for k in state_dict if "vblora_logits" in k})
        else:
...
def set_peft_model_state_dict(
    ...
    if config.peft_type in (
        ...
        if config.peft_type == PeftType.VBLORA and config.save_topk_logits:
            for k, v in state_dict.items():
                if 'topk_indices' in k:
                    new_key = k.replace('_topk_indices', '')
                    values = state_dict[new_key + '_topk_values']
                    state_dict[new_key] = model.topk_to_logits(v, values)
                    state_dict.remove(k)
                    state_dict.remove(new_key + '_topk_values')
