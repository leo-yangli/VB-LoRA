...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    num_vectors: int = field(
        default=90,
        metadata={"help": "Number of vectors in the vector bank."}
    )

    learning_rate_vector_bank: float = field(default=1e-3, metadata={"help": "The initial learning rate for vector bank."})
    learning_rate_logits: float = field(default=1e-2, metadata={"help": "The initial learning rate for logits."})
...
def get_accelerate_model(args, checkpoint_dir):
    ...
    if not args.full_finetune:
        if checkpoint_dir is not None:
            ...
            config = VBLoRAConfig(r=args.lora_r, vblora_dropout=args.lora_dropout, target_modules=modules, 
                                  num_vectors=args.num_vectors, task_type="CAUSAL_LM",)
